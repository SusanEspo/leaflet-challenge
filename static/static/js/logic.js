...
d3.json(url).then(function (data) {
  ...
  function createFeatures(earthquakeData) {
    ...
    function onEachFeature(feature, layer) {
      layer.bindPopup(`<h2></h2<>br<h3>${feature.properties.mag}</h3><br><p>${feature.properties.depth}</p>`);
    }
...
// let info = L.control({
//   postition:"bottonright"
// })
      
// info.onAdd = function() {
// let div = L.DomUtil.create("dev","legend");
//       return div;
// };
...
// info.addTo(map);
  ...
  labels = ['<b>Quakes</b>'];
  magit =  ['10-30','30-60','60-70','70-90','99+']
      // loop through our density intervals and generate a label with a colored square for each interval
      div.innerHTML ='';

      for (var i = 0; i < magit.length; i++) {
        labels.push(
            '<i class="circle" style="background:' + getColor(magit[i]) + '"></i> ' +
        (magit[i] ? magit[i] : '+'));
